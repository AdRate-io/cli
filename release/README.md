...
因此 `LICENSE`、`CODEOWNERS` 等一律不得由维护者在公开仓库直接添加；确有必要时先把文件加入私有源并同步扩展 `scripts/public-mirror.mjs` 的 allowlist，再由镜像写入。注意 `LICENSE` 还会被 npm 无条件打进 tarball（与 `package.json.files` 无关），新增它必须同时重签精确 14 项 tarball 合同。仓库当前以 `package.json.license = "MIT"` 声明授权，暂不随包分发许可证正文。
...
3. 先完成 prerelease channel 的 4 项 required gate，并由独立 reviewer 审批声明式 pin，再推 prerelease semver 的 `v<package-version>` tag；workflow 自动使用 npm dist-tag `next`。
